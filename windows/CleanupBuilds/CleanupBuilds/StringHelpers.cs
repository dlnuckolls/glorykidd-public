...
namespace CleanupBuilds {
  public static class StringHelpers {
    ...
    public static string ReadSqlPhone(this string p) {
      p = p.FixSqlPhone();
      if (p.IsNullOrEmpty())
        return string.Empty;
      else if (p.Length < 10)
        ...
        return "({0}) {1}-{2}".FormatWith(p.Substring(0, 3), p.Substring(3, 3), p.Substring(6));
    }
    #endregion
    ...
    /// <summary>
    /// Split a long string into managable chunks
    /// </summary>
    ...
    public static List<string> SplitBy(this string str, int chunkLength) {
      ...
      for (int i = 0; i < str.Length; i += chunkLength) {
        if (chunkLength + i > str.Length) chunkLength = str.Length - i;
        rtn.Add(str.Substring(i, chunkLength));
      }
      return rtn;
    }
    /// <summary>
