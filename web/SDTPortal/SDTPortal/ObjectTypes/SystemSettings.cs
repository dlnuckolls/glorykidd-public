...
namespace SDTPortal {
  public class SystemSettings {
    ...
    public void LogError(string module, Exception error, string documentSubId = "") {
      SqlHelpers.Insert(SqlStatements.SQL_LOG_EXCEPTION.FormatWith(DateTime.Now.ConvertSqlDateTime(), module.FixSqlString(), error.Message.FixSqlString(), error.StackTrace.FixSqlString(), (documentSubId.IsNullOrEmpty()) ? "NULL" : "'{0}'".FormatWith(documentSubId)));
      if(!error.InnerException.IsNullOrEmpty())
        SqlHelpers.Insert(SqlStatements.SQL_LOG_EXCEPTION.FormatWith(DateTime.Now.ConvertSqlDateTime(), module.FixSqlString(), error.InnerException.Message.FixSqlString(), error.InnerException.StackTrace.FixSqlString(), (documentSubId.IsNullOrEmpty()) ? "NULL" : "'{0}'".FormatWith(documentSubId)));
    }
