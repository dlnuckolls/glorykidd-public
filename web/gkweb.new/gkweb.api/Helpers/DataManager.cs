...
{
  /// <summary>
  /// Safely get the value from a SqlDataReader column (accounting for NULL values)
  /// </summary>
  /// <typeparam name="T">DataType of column</typeparam>
  /// <param name="reader">SqlDataReader to read from</param>
  /// <param name="nameOfColumn">Column name to read the value from</param>
  /// <returns>The typed value from a specific column of the SqlDataReader</returns>
...
#pragma warning restore CS8603 // Possible null reference return.
  ...
  internal static async Task<List<Staff>> GetStaffRecords(bool showDeleted = false)
  {
    var rtn = new List<Staff>();
    await using var connection = SqlHelper.GetSqlConnection();
    await connection.OpenAsync();

    var sql = "SELECT [Id], [DisplayName], [Title], [ImgUrl] FROM [dbo].[StaffRecords] {0};".FormatWith(!showDeleted ? string.Empty : "WHERE [Deleted] = 1");
    ...
    while (reader.Read())
    {
      staff = new Staff()
      {
        ...
        Title = SafeGet<string>(reader, "DisplayName"),
        Summary = SafeGet<string>(reader, "Title"),
        ImageUrl = SafeGet<string>(reader, "ImgUrl")
      };
      rtn.Add(staff);
    }
    return rtn;
